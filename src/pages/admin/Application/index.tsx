...
const ApplicationManagement: React.FC = () => {
  ...
  const [nganhMap, setNganhMap] = useState<{[key: string | number]: string}>({});
  ...
  const fetchSchoolsAndNganh = async () => {
    try {
      ...
      // Lấy danh sách ngành
      // eslint-disable-next-line @typescript-eslint/no-shadow
      const nganhMap: Record<string | number, string> = {};
      for (let i = 1; i <= 5; i++) {
        const result = await getNganhByTruongId(i);
        if (result.success && result.data) {
          result.data.forEach((nganh: { id: string | number; ten_nganh: string; }) => {
            ...
            <div style={{ margin: '8px 0' }}>
              ...
              {selectedApplication.file_minh_chung ? (
                <div style={{ marginTop: 8 }}>
                  {selectedApplication.file_minh_chung.split(',').map((file: string | URL | undefined, index: React.Key | null | undefined) => {
                    const fileString = file?.toString() || '';
                    const fileName = fileString.trim().split('/').pop() || fileString;
                    const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(fileString);
